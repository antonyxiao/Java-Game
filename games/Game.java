...
public class Game extends Canvas {
        ...
        private double moveSpeed = 200; // hor. vel. and ver. vel. of ship (px/s)
        private long lastFire = 500; // time last shot fired
        private long firingInterval = 400; // interval between shots (ms)
        ...
        Point p = null; // gets mouse coordinates
        private int mouseX = 0; // mouse y coordinate
        private int mouseY = 0; // mouse x coordinate

        private int distanceX = 0; // X distance from player to mouse
        private int distanceY = 0; // Y distance from player to mouse
        
        private float rotation = 0; // angle of player rotation to the mouse
    	...
    	private void initEntities() {
              ...
              ship = new ShipEntity(this, "sprites/ship.png", 350, 250);
              entities.add(ship);
         ...
         */
         ...
         public void notifyDeath() {
           message = "You DEAD Human! Try again?";
        ...
        public void tryToFire() {
          ...
          shot = new ShotEntity(this, "sprites/shot.png", 
                            (int)bulletX, (int)bulletY, rotation);
          entities.add(shot);
        } // tryToFire
	...
	public void gameLoop() {
          ...
          while (gameRunning) {
			...
			AffineTransform oldXForm = g.getTransform();// tracks old graphic form
			
            // refreshes mouse position
            ...
            if(mouseX >= 0 && mouseY >= 0 && mouseX <= 800 && mouseY <= 600) {
            	// gets angle of mouse relative to player
            	distanceX = mouseX - ship.getX() - 40;
            	distanceY = mouseY - ship.getY() - 73;
            }
            
            rotation = (float)Math.atan2(distanceY, distanceX);
            
            //System.out.println("distanceX :" + distanceX + " | distanceY: " + distanceY + " || Rotation: " + rotation);
            ...
            for (int i = 0; i < entities.size(); i++) {
        		Entity entity = (Entity) entities.get(i);
        		int shotIndex = entities.indexOf(shot);
        		
        		if (i == 0) {

        			g.rotate(rotation+(Math.PI/2), ship.getX() + 43, ship.getY() + 75);
            		entity.draw(g);
            		
        		}else if (i == shotIndex){
        			
        			g.rotate(rotation+(Math.PI/2), shot.getX(), shot.getY());
            		entity.draw(g);
        		...
        		g.setTransform(oldXForm);// removes the rotation for the text
           ...
           if (waitingForKeyPress) {
            ...
            // if mouse left clicked, try to fire
            if (leftClicked) {
              tryToFire();
            } // if
